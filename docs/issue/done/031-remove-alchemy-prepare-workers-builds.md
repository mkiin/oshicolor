...
- [x] `package.json` から `alchemy` 依存と `deploy` / `destroy` スクリプトが削除されている
- [x] `src/infrastructures/alchemy/` ディレクトリが repo から消えている
- [x] `.env.example` から `ALCHEMY_PASSWORD` と `ALCHEMY_STATE_TOKEN` が削除されている
- [x] Cloudflare 側に残った alchemy state (KV / R2 / D1) が削除されている
- [x] `.github/workflows/deploy.yml` が無効化または削除されている
- [x] `wrangler.jsonc` が Workers Builds で deploy できる最小構成 (name、main、compatibility_date) になっている
- [x] 手元の `wrangler deploy` が成功する
- [x] `pnpm dev` (vite dev) が動く
