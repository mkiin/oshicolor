...
const generateBlock = (name: string, result: ExtractionResult, blockY: number): string => {
  const lines: string[] = [];
...
const generateSvg = (blocks: Array<{ name: string; result: ExtractionResult }>): string => {
  ...
  const parts = [
    ...
    ...blocks.map(({ name, result }, i) => generateBlock(name, result, PAD + i * BLOCK_H)),
    "</svg>",
  ];
  return parts.join("\n");
};
      ...
      async ({ src, opts, optsBase, domCount }) => {
        ...
        const w = window as unknown as {
          ...
          __getSwatches: (
            img: HTMLImageElement,
            opts: unknown,
          ) => Promise<Record<string, { color: { hex(): string; isDark: boolean } } | null>>;
        };
